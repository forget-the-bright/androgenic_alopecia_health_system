...
-- 初始化系统管理员账号（密码：admin123，MD5 加密）
INSERT INTO `sys_user` (`username`, `password`, `real_name`, `role`, `status`)
VALUES ('admin', 'e10adc3949ba59abbe56e057f20f883e', '系统管理员', 1, 1);

-- 初始化测试用户账号（密码：user123，MD5 加密）
INSERT INTO `sys_user` (`username`, `password`, `real_name`, `role`, `status`)
VALUES ('user', 'e10adc3949ba59abbe56e057f20f883e', '测试用户', 0, 1);

-- 初始化系统基础配置
INSERT INTO `sys_config` (`config_key`, `config_value`, `config_type`, `remark`)
VALUES
('minio.endpoint', 'http://localhost:9000', 'minio', 'MinIO 服务地址'),
('minio.access-key', 'minioadmin', 'minio', 'MinIO 访问密钥'),
('minio.secret-key', 'minioadmin', 'minio', 'MinIO 秘密密钥'),
...
('ai.analysis.key', 'your-api-key', 'ai', 'AI 分析接口密钥'),
('system.login.timeout', '7200', 'system', '登录会话超时时间（秒）'),
('system.upload.max-size', '10485760', 'system', '文件上传大小限制（字节）');
